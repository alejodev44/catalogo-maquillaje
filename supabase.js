...
async function fetchProductosPreciosProblematicos() {
  ...
  const { data, error } = await db
    ...
    .or("precio_referencia.eq.0,precio_referencia.is.null")
    .order("marca", { ascending: true })
    .order("nombre", { ascending: true });
  ...
  const sinPrecioRef = (data || []).filter(p => !p.precio_referencia || p.precio_referencia === 0);
  
  // Productos donde mi_precio = precio_mayorista (sin margen)
  const conPrecioMayorista = (data || []).filter(p => 
    p.precio_referencia && p.precio_referencia > 0 && 
    p.mi_precio === p.precio_mayorista
  );
