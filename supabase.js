...
async function fetchProductosPreciosProblematicos() {
  ...
  // Traer TODOS los productos y luego filtrar en memoria
  const { data, error } = await db
    .from("productos")
  ...
  if (todos.length > 0) {
    ...
    console.log("🔍 Primer producto (verificar tipos):", {
      id: primero.id,
      nombre: primero.nombre,
      precio_referencia: primero.precio_referencia,
      precio_referencia_tipo: typeof primero.precio_referencia,
      precio_mayorista: primero.precio_mayorista,
      precio_mayorista_tipo: typeof primero.precio_mayorista,
      mi_precio: primero.mi_precio,
      mi_precio_tipo: typeof primero.mi_precio
    });
    ...
    console.log(`Productos con precio_referencia NULL: ${conNull.length}`);
    console.log(`Productos con precio_referencia = 0: ${conCero.length}`);
    console.log(`Productos con precio_referencia = "0": ${conCeroString.length}`);
    
    if (conNull.length > 0) {
      console.log("Ejemplo con NULL:", conNull[0]);
    }
    if (conCero.length > 0) {
      console.log("Ejemplo con 0:", conCero[0]);
    }
  }
