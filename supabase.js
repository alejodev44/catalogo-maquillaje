...
async function fetchProductosPreciosProblematicos() {
  ...
  // Mostrar algunos ejemplos de productos con sus precios
  console.log("Ejemplos de datos:", todos.slice(0, 3).map(p => ({
    id: p.id,
    nombre: p.nombre,
    precio_mayorista: p.precio_mayorista,
    mi_precio: p.mi_precio,
    precio_referencia: p.precio_referencia
  })));
  
  // Separar en dos categorías
  const sinPrecioRef = todos.filter(p => {
    const tieneProblema = !p.precio_referencia || p.precio_referencia === 0;
    return tieneProblema;
  });
  
  console.log(`Productos sin precio_referencia: ${sinPrecioRef.length}`);
  if (sinPrecioRef.length > 0) {
    console.log("Primeros ejemplos:", sinPrecioRef.slice(0, 2));
  }
  
  // Productos donde mi_precio = precio_mayorista (sin margen)
  const conPrecioMayorista = todos.filter(p => {
    const tieneRef = p.precio_referencia && p.precio_referencia > 0;
    const mismoPrecio = Number(p.mi_precio) === Number(p.precio_mayorista);
    const tieneProblema = tieneRef && mismoPrecio;
    return tieneProblema;
  });
  
  console.log(`Productos con precio = mayorista: ${conPrecioMayorista.length}`);
  if (conPrecioMayorista.length > 0) {
    console.log("Primeros ejemplos:", conPrecioMayorista.slice(0, 2));
  }
